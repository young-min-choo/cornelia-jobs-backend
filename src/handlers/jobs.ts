...
export async function createJob(request: Request<Job, {}, CreateJobDto>, response: Response<Job>) {
    ...
    // save job
    const db = await dbPromise
    db.run('INSERT INTO Job (id, title, location, salary, postTime, description) VALUES (?, ?, ?, ?, ?, ?)',
        [job.id, job.title, job.location, job.salary, postTime.toISOString(), job.description],
        function (err: string) {
            if (err) {
